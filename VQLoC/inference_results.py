...
def get_results(annotations, config):
    ...
    task_queue = mp.Queue()
    for clip_name, annot in annotations.items():
        task = Task(config, clip_name, annot)
        task_queue.put(task)
...
def format_predictions(annotations, predicted_rts):
    ...
    clip_predictions = dict()
    for clip_name, c in annotations.items():
    # for c in v["clips"]:
        # clip_predictions = {"clip_uid": clip_name, "predictions": []}
        clip_predictions[clip_name] = {"predictions": []}
        for a in c["annotations"]:
            ...
            clip_predictions[clip_name]["predictions"].append(apred)
...
if __name__ == '__main__':
    ...
    config.inference_cache_path = os.path.join(output_dir, f'inference_cache_{mode}_1')
    ...
    annotation_path = os.path.join('/data/jason/DLCV_vq2d_data', 'vq_{}.json'.format(mode))
    ...
    if not args.debug:
        with open(config.inference_cache_path + '_results_1.json', 'w') as fp:
